...
% --- Left column (top to bottom) ---
        \node[decision] (datatype)    at (0, 8)  {Welke soort data?};
        \node          (aantallen)   at (0, 6)  {Aantallen};
        \node          (distributies)at (0, 4)  {Distributies};
        \node          (proporties)  at (0, 2)  {Proporties};
        \node          (geospatial)  at (0, 0)  {Geospatiaal};

% --- Middle ---
        \node[decision] (variables)  at (5, 6)  {Hoeveel variabelen?};

% --- Variable choices ---
        \node (onevar) at (9, 7) {1 variabele};
        \node (twovar) at (9, 5) {2 variabelen};

% --- Plot types ---
        \node (onevarplots) at (13, 7) {Staafdiagram, heatmap, \ldots};
        \node (twovarplots) at (13, 5) {Scatterplot, lijngrafiek, \ldots};

        \node (distplots) at (5, 4) {Histogram, density plot, boxplot, \ldots};
        \node (propplots) at (5, 2) {Pie chart, stacked bar chart, treemap, \ldots};
        \node (geoplots)  at (5, 0) {Kaart, choropleth, \ldots};

% --- Uncertainty ---
        \node[decision] (uncertainty) at (13, 3) {Onzekerheid in de data?};
        \node (uncplots)             at (13, 1) {Error bars, spaghetti plot, \ldots};

% --- Arrows: vertical ---
        \draw[arrow] (datatype) -- (aantallen);
        \draw[arrow] (aantallen) -- (distributies);
        \draw[arrow] (distributies) -- (proporties);
        \draw[arrow] (proporties) -- (geospatial);

% --- Arrows: aantallen path ---
        \draw[arrow] (aantallen) -- (variables);
        \draw[arrow] (variables) -- (onevar);
        \draw[arrow] (variables) -- (twovar);
        \draw[arrow] (onevar) -- (onevarplots);
        \draw[arrow] (twovar) -- (twovarplots);

% --- Arrows: other paths ---
        \draw[arrow] (distributies) -- (distplots);
        \draw[arrow] (proporties) -- (propplots);
        \draw[arrow] (geospatial) -- (geoplots);

% --- Uncertainty arrows ---
        \draw[arrow] (distplots) -- (uncertainty);
        \draw[arrow] (propplots) -- (uncertainty);
        \draw[arrow] (geoplots) -- (uncertainty);
        \draw[arrow] (onevarplots) -- (uncertainty);
        \draw[arrow] (twovarplots) -- (uncertainty);
        \draw[arrow] (uncertainty) -- (uncplots);
    ...
    \begin{itemize}
        \item Slecht ontwerp, zoals onduidelijke labels, slechte kleurkeuzes en onjuiste schalen kunnen misleiden.
        \item proporties kunnen vertekend worden weergegeven, bijvoorbeeld door de y-as niet bij 0 te laten beginnen.
        \item 3D-effecten kunnen de verhoudingen vertekend weergeven.
    \end{itemize}
