...
\def\documentTitle      {Informatie Uitwisseling} % replace with actual document title
...
% ====================================================
    ...
    * De ``?name'' betekendt hetzelde als ``<http://xmlns.com/foaf/0.1/name>'' maar dan korter geschreven.
    ...
    \section{Afstemming in Communicatie}
    ...
    \subsection{Beperkingen van klassieke modellen}
    ...
    \subsection{Drie pijlers van afstemming}

    \textbf{1. Achtergrondinformatie}
    \begin{itemize}
        \item Gesprekspartners hebben kennis nodig van relevante objecten, relaties, aannames en spelregels
        \item Deze informatie is grotendeels impliciet en blijft onuitgesproken
        \item Veel wordt verondersteld: niemand hoeft uit te leggen dat je met griep thuisblijft
    \end{itemize}

    \textbf{2. Activatie}
    \begin{itemize}
        \item Achtergrondinformatie moet op het juiste moment beschikbaar zijn
        \item Dit gebeurt via aandacht, taalkeuze, framing en focussering
        \item Voorbeeld: een instructietekst wordt pas begrijpelijk als je weet dat het over vliegeren gaat
    \end{itemize}

    \textbf{3. Common ground \& coördinatie}
    \begin{itemize}
        \item \textit{Common ground}: informatie waarvan beide partijen weten dat ze die delen, en weten dat de ander dat ook weet
        \item Dit is recursief, dynamisch en altijd onvolledig
        \item Coördinatie via bevestigen, verifiëren en corrigeren houdt de gemeenschappelijke basis op orde
        \item Uitingen als ``bedoel je dat?'' zijn geen beleefdheden maar cruciale mechanismen
    \end{itemize}

    \subsection{Wanneer het misgaat}

    Verkeerde aannames over wat de ander weet kunnen ernstige gevolgen hebben, bijvoorbeeld in:
    \begin{itemize}
        \item Luchtvaart, zorg en helpdesks
        \item Ontwerp van interfaces (stereotiepe aannames over gebruikers)
        \item Hiërarchische situaties (niet durven vragen of corrigeren)
    \end{itemize}

    \ruleline{Alles hierboven is voor de midterm, alles hieronder is voor de final}


    \section[visualisatietechnieken - \textbf{InformatieVisualisatie}]{visualisatietechnieken}

    \subsection{Definitie}
    ``Een visuele representatie van gegevens om mensen sneller taken uit te kunnen laten voeren'' \& ``punten, lijnen, coordinaten, kleuren, vormen en texturen gebruiken om data te representeren''

    Je kan denken aan bijvoorbeeld (lijn-, staaf-, cirkel- en spreidings-)diagrammen, maar ook aan meer complexe visualisaties zoals heatmaps, boomstructuren en netwerkdiagrammen.
    Dit kan je dan allemaal gebruiken om dashboards, rapporten en presentaties (overzichtelijker) te maken.

    Informatievisualisatie is \textbf{niet} gestructureerde data.
    Wetenschapelijke visualisatie is \textbf{wel} gestructureerde data.
    Dit zijn allebij onderdelen van het grote Data Visualisatie veld.

    Het is een belangrijk, omdat het helpt om patronen, trends en inzichten te ontdekken die anders moeilijk te zien zouden zijn in grote datasets.
    Ook kan je door middel van verschillende visualisaties van dezelfde data verschillende inzichten krijgen.
    ...
    \begin{figure}[!ht]
        ...
        \resizebox{0.25\textwidth}{!}{%
            \begin{tikzpicture}
                ...
                \draw  (1,9.75) rectangle (1,9.75);



                \draw  (0,8.75) rectangle (3.75,3.75);
                \draw  (0.25,8.75) rectangle (0.25,8.75);
                \draw  (0.5,8.25) circle (0.25cm);
                \draw  (1,7.5) circle (0.25cm);
                \draw  (2.25,7.5) circle (0.25cm);
                \draw  (2.25,6.75) circle (0.25cm);
                \draw  (1.25,6) circle (0cm);
                \draw  (1,6.25) circle (0.25cm);
                \draw  (1.5,6) circle (0.25cm);
                \draw  (2.5,5.5) circle (0.25cm);
                \draw  (1.75,5) circle (0.25cm);
                \draw  (0.75,5) circle (0.25cm);
                \draw  (3,6.75) circle (0.25cm);
                \draw  (2,8.25) circle (0.25cm);
                \draw  (2.75,8.25) circle (0.25cm);
                \draw  (3,4.25) circle (0cm);
                \draw  (2.5,4.5) circle (0.25cm);
                \draw  (1.25,4.25) circle (0.25cm);
                \draw  (1.25,7) circle (0.25cm);
                \draw  (5,8.75) rectangle (8.75,3.75);
                \draw  (5.5,8.25) circle (0.25cm);
                \draw  (6.25,8.25) circle (0.25cm);
                \draw  (7,8.25) circle (0.25cm);
                \draw  (5.5,7.5) circle (0.25cm);
                \draw  (6.25,7.5) circle (0.25cm);
                \draw  (7,7.5) circle (0.25cm);
                \draw  (5.5,6.75) circle (0.25cm);
                \draw  (6.25,6.75) circle (0.25cm);
                \draw  (7,6.75) circle (0.25cm);
                \draw  (5.5,6) circle (0.25cm);
                \draw  (6.25,6) circle (0.25cm);
                \draw  (7,6) circle (0.25cm);
                \draw  (5.5,5.25) circle (0.25cm);
                \draw  (6.25,5.25) circle (0.25cm);
                \draw  (7,5.25) circle (0.25cm);
            \end{tikzpicture}
        }%

        \caption{Slechte en goede manier om cirkel hoeveelheid te visualiseren}
        \label{fig:circle_visualisatie}
    \end{figure}
    ...
    \subsection{geschiendenis}
    (Geen idee of het nuttig is)\\
    In 1786 maakte William Playfair de eerste grafieken.
    In 1854 gebruikte John Snow een kaart om een cholera-uitbraak in Londen te analyseren.
    In 1958 ontwikkelde Florence Nightingale de coxcomb chart om sterfteoorzaken in het leger te visualiseren.
    In 1869 introduceerde Charles Minard de beroemde kaart van Napoleon's Russische veldtocht.
    In 1933 maakte Harry Beck de eerste metrokaart van Londen.

    \subsection{Algemene visualisatie principes}
    Er zijn een hele boel dingen die je kan doen om je visualisaties beter te maken

    \textbf{Vermijd onnodige 3D-effecten}.
    Dit maakt het moeilijker om data te interpreteren én door diepte printen gaat er informatie verloren/worden de verhoudingen vertekend.
    \textbf{Pie charts werken vaak niet goed}.
    Het is lastig om oppervlakte te vergelijken, vooral bij meerdere segmenten.
    Om deze reden moet je \textbf{oppervlakte vergelijken vermijden}.


    Edward Tufte heeft een aantal principes opgesteld voor effectieve informatievisualisatie:
    \begin{itemize}
        \item \textbf{Grafische integriteit}: Zorg ervoor dat de visualisatie de data nauwkeurig en eerlijk weergeeft, zonder misleiding.
        \item \textbf{Designesthetiek}: Maak de visualisatie visueel aantrekkelijk en gemakkelijk te begrijpen.
    \end{itemize}

    Hij zegt bij Designesthetiek:

    \begin{enumerate}
        \item Laat vooral de data zien, geen poespas.
        \item Maximaliseer redelijkerwijs de \textbf{data-ink ratio} (de hoeveelheid inkt die daadwerkelijk data representeert versus de totale hoeveelheid inkt gebruikt in de visualisatie). $Data\text{-}ink\ ratio = \frac{Data\ ink}{Total\ ink}$
        \item Verwijder inkt die geen data representeert (zoals overbodige lijnen, kaders en decoraties).
        \item Kijk en doe opnieuw (iteratief proces).
    \end{enumerate}

    \begin{figure}[h]
        \centering
        \includegraphics[width=0.5\linewidth]{image/bad_and_good_vis}
        \caption{Slechte en goede visualisatie}
        \label{fig:bad_and_good_vis}
    \end{figure}

    Hier zie je een voorbeeld van een slechte en een goede visualisatie.
    De slechte visualisatie heeft veel onnodige elementen die de data verbergen en heel veel poespas toevoegen.
    De goede visualisatie is veel eenvoudiger en laat de data duidelijker zien.
    Bij de linker is de data-ink ratio heel laag, terwijl die bij de rechter hoog is.

    \subsection{Mark en channels}

    \subsubsection{Marks}
    zijn de basisvormen die worden gebruikt om data te representeren in een visualisatie.
    De meest voorkomende marks zijn punten, lijnen en vlakken.
    Deze marks kunnen worden gecombineerd en aangepast om verschillende soorten visualisaties te creëren, zoals scatterplots, lijndiagrammen en staafdiagrammen.

    Marks kunnen punten zijn (zoals in een scatterplot), lijnen (zoals in een lijndiagram) of vlakken (zoals in een staafdiagram).
    Ook kan je Marks as Links gebruiken, dit zijn lijnen die twee of meer punten verbinden om relaties tussen data-elementen te tonen (zoals in een netwerkdiagram).

    Marktypes zijn voor hoeveel Dimensies:
    \begin{itemize}
        \item 0D: punten (groote kan variëren, alleen locatie is relevant)
        \item 1D: lijnen (lengte kan variëren, locatie en oriëntatie zijn relevant)
        \item 2D: vlakken (oppervlakte kan variëren, locatie, oriëntatie en vorm zijn relevant)
        \item 3D: volumes (volume kan variëren, locatie, oriëntatie en vorm zijn relevant)
    \end{itemize}

    \subsubsection{Channels}
    zijn de visuele eigenschappen die worden gebruikt om marks te coderen en informatie over te brengen.
    Veelvoorkomende channels zijn positie, kleur, vorm, draaiing, grootte, oppervlakte, volume en textuur.
    Deze channels kunnen worden aangepast om verschillende aspecten van de data te benadrukken en om patronen en trends te identificeren.
    Bijvoorbeeld, in een scatterplot kan de positie van de punten de waarden van twee variabelen representeren, terwijl de kleur van de punten een derde variabele kan aangeven.

    \begin{multicols}{2}

        Dit is de lijst van beste naar slechtste channels (voor georganiseerde data):
        \begin{enumerate}
            \item Positie op een gemeenschappelijke schaal
            \item Positie op niet-gemeenschappelijke schalen
            \item Lengte
            \item Hoek/oriëntatie
            \item Oppervlakte (2D)
            \item diepte (3D)
            \item Kleurintensiteit
            \item kleursaturatie
            \item ronding
            \item volume
        \end{enumerate}
    ...
    \section{Theoretische fundamenten}

    \subsection{Visuele perceptie}
    Mensen zijn in sommige soorten perceptie erg goed, zoals het herkennen van patronen, kleuren en bewegingen.
    Maar op sommige erg slecht, zo zijn we niet goed in het inschatten van oppervlakten, volumes en hoeken.

    Dit kan je ook in je voordeel gebruiken.
    Zo kan je gebruik maken van ``popout'' Dit is wanneer een object zich onderscheidt van de rest door een uniek kenmerk, zoals kleur, vorm of grootte.
    Hierdoor valt het object direct op en trekt het de aandacht van de kijker.
    Bijvoorbeeld, in een afbeelding met allemaal blauwe cirkels, zal een rode cirkel direct opvallen en de aandacht trekken.

    \subsection{kleurgebruik}
    ...
    \begin{multicols}{2}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=\linewidth]{image/kleurenblindheid}
            \caption{}
            \label{fig:kleurenblindheid}
        \end{figure}

        \begin{figure}[H]
            \centering
            \includegraphics[width=\linewidth]{image/kleurenblindheid_pallete}
            \caption{}
            \label{fig:kleurenblindheid_pallete}
        \end{figure}

    \end{multicols}

    Hier zie je heel goed dat er mensen zijn die rood en groen niet goed kunnen onderscheiden.
    Daarom is het belangrijk om kleuren te kiezen die voor iedereen goed te onderscheiden zijn.
    Hiervoor zijn verschillende kleurpalleten voor om goede kleuren te kiezen.

    Je kan verschillende soorten oplossingen gebruiken om kleurenblindheid te omzeilen, zoals:
    \begin{itemize}
        \item Gebruik patronen of texturen in plaats van alleen kleuren.
        \item Gebruik symbolen of vormen om verschillende categorieën aan te duiden.
        \item Gebruik kleuren die goed te onderscheiden zijn voor mensen met kleurenblindheid, zoals blauw en geel.
    \end{itemize}

    \textbf{Simultaan contrast}: Dit is een optische illusie waarbij de kleur van een object wordt beïnvloed door de kleuren eromheen.
    Bijvoorbeeld, een grijs vlak kan er lichter of donkerder uitzien afhankelijk van de achtergrondkleur.
    Dit kan leiden tot misinterpretaties van de data als de kleuren niet goed gekozen zijn.

    \textbf{Chromostereopsis}: Dit is een optische illusie waarbij kleuren op verschillende afstanden lijken te liggen.
    Bijvoorbeeld, rode objecten lijken dichterbij te zijn dan blauwe objecten.

    \subsection{Klassieke visualisatie-types}
    ...
    \begin{figure}[!ht]
        ...
        \resizebox{1\textwidth}{!}{%
            \begin{tikzpicture}
                ...
                \draw [ fill={rgb,255:red,124; green,204; blue,254} , rounded corners = 6.0] (2.5,12.25) rectangle node {\LARGE Welke soort data?} (8.75,11);
                \draw [ rounded corners = 6.0] (2.5,9.75) rectangle node {\LARGE Aantallen} (8.75,8.5);
                \draw [ rounded corners = 0.0] (3.5,11.5) rectangle (3.5,11.5);
                \draw [ rounded corners = 0.0] (3.5,11.75) rectangle (3.5,11.75);
                \draw [ rounded corners = 0.0] (3.75,11.75) rectangle (3.75,11.75);
                \draw [ rounded corners = 6.0] (2.5,7.25) rectangle node {\LARGE Distributies} (8.75,6);
                \draw [ rounded corners = 6.0] (2.5,4.75) rectangle node {\LARGE Proporties} (8.75,3.5);
                \draw [ rounded corners = 6.0] (2.5,2.25) rectangle node {\LARGE Geospatiaal} (8.75,1);
                \draw [ fill={rgb,255:red,124; green,204; blue,254} , rounded corners = 6.0] (10,9.75) rectangle node {\LARGE Hoeveel variabelen?} (16.25,8.5);
                \draw [ rounded corners = 6.0] (17.5,12.25) rectangle node {\LARGE 1 variabele} (23.75,11);
                \draw [ rounded corners = 6.0] (17.5,9.75) rectangle node {\LARGE 2 Variabelen} (23.75,8.5);
                \draw [short] (5.5,11) -- (5.5,9.75);
                \draw [short] (5.5,8.5) -- (5.5,7.25);
                \draw [short] (5.5,6) -- (5.5,4.75);
                \draw [short] (5.5,3.5) -- (5.5,2.25);
                \draw [short] (16.25,9.25) -- (17.5,9.25);
                \draw [short] (16.75,9.25) -- (16.75,11.75);
                \draw [short] (16.75,11.75) -- (17.5,11.75);
                \node [font=\LARGE] at (14.5,6.5) {\textbf{Histogram, density plot, boxplot …}};
                \node [font=\LARGE] at (15,4) {\textbf{Pie chart, stacked bar chart, treemap …}};
                \node [font=\LARGE] at (12.75,1.5) {\textbf{Kaart, chloropleth …}};
                \draw (10,6) to[short] (19,6);
                \draw (10,3.5) to[short] (20,3.5);
                \draw (10,1) to[short] (15.75,1);
                \draw [->, >=Stealth] (8.75,6.75) -- (9.75,6.75);
                \draw [->, >=Stealth] (9.75,6.75) -- (9.75,6.75);
                \draw [->, >=Stealth] (8.75,4.25) -- (9.75,4.25);
                \draw [->, >=Stealth] (8.75,1.75) -- (9.75,1.75);
                \draw [->, >=Stealth] (8.75,9.25) -- (9.75,9.25);
                \node [font=\LARGE] at (28.25,11.5) {\textbf{Staafdiagram, heatmap …}};
                \node [font=\LARGE] at (28.25,9) {\textbf{Scatterplot, lijngrafiek …}};
                \draw [->, >=Stealth] (23.75,11.75) -- (24.75,11.75);
                \draw [->, >=Stealth] (23.75,9.25) -- (24.75,9.25);
                \draw [short] (31.75,11.75) -- (33,11.75);
                \draw [short] (31.75,9.25) -- (33,9.25);
                \draw [short] (33,11.75) -- (33,7.25);
                \draw [ fill={rgb,255:red,124; green,204; blue,254} , rounded corners = 6.0] (30,7.25) rectangle node {\LARGE Onzekerheid i/d data?} (36.25,6);
                \draw [short] (30,6.75) -- (19.25,6.75);
                \draw [short] (21.75,6.75) -- (21.75,1.75);
                \draw [short] (21.75,1.75) -- (15.75,1.75);
                \draw [short] (21.75,4.25) -- (20.25,4.25);
                \draw [->, >=Stealth] (33,6) -- (33,5);
                \node [font=\LARGE] at (33,4) {\textbf{Error bars, spaghetti plot …}};
                \draw [short] (29.5,3.5) -- (36.5,3.5);
                \draw [short] (25,8.5) -- (31.5,8.5);
                \draw [short] (25,11) -- (31.75,11);
            \end{tikzpicture}

        }%
    \end{figure}

    \section{Het geneste model}

    Tamara Munzner heeft een model om visualisaties te ontwerpen én te valideren.
    Het komt er op neer dat je vier niveaus hebt waarop je fouten kan maken, en dat je op elk niveau moet valideren of je ontwerp klopt.
    Dit moet je doen voordat je verder gaat naar het volgende niveau.
    Doe je dit niet, dan gaat het altijd mis op volgende niveaus.

    Het model bestaat uit vier geneste niveaus.
    Elk niveau levert input aan het volgende, en elk niveau kent zijn eigen typische fouten en bijbehorende manieren om die fouten te toetsen.

    \subsection{De vier niveaus}

    \begin{enumerate}
        \item \textbf{Domeinprobleem en data}
        Eerst moet je begrijpen wat gebruikers daadwerkelijk doen en welke data daarbij hoort, in hun eigen taal en context.
        Het risico hier is simpel: je lost een probleem op dat zij helemaal niet hebben.
        Validatie gebeurt door observatie, interviews en veldstudies.

        \item \textbf{Abstractie naar operaties en datatypen}
        Vervolgens vertaal je het domeinprobleem naar generieke operaties (zoals vergelijken, filteren, clusteren) en datatypen (tabellen, grafen, tijdreeksen).
        Dit is vaak het zwakste punt in visualisatie-ontwerp.
        De klassieke fout: je kiest een verkeerde abstractie, waardoor de visualisatie per definitie niet helpt.
        Dit kun je alleen valideren door echte gebruikers hun eigen werk te laten doen met het systeem.

        \item \textbf{Visuele encodering en interactie}
        Pas hier beslis je hoe iets eruitziet en hoe gebruikers ermee werken.
        De vraag is niet of het mooi is, maar of het de gekozen abstractie effectief overbrengt.
        Validatie kan via ontwerp-argumentatie, perceptuele principes, labstudies (tijd en fouten), en analyse van resultaatbeelden.

        \item \textbf{Algoritmes}
        Het binnenste niveau gaat over performance en correctheid: is het snel genoeg, schaalbaar, en technisch juist?
        Validatie gebeurt via complexiteitsanalyse en metingen van tijd en geheugen.
    \end{enumerate}

    \subsection{Kernidee: fouten werken door}

    De niveaus zijn genest, niet onafhankelijk. Een fout hogerop besmet alles eronder:
    \begin{itemize}
        \item een verkeerd probleem $\rightarrow$ niemand gebruikt je tool;
        \item een verkeerde abstractie $\rightarrow$ je laat gebruikers het verkeerde zien;
        \item een slechte encodering $\rightarrow$ mensen begrijpen het niet;
        \item een traag algoritme $\rightarrow$ het systeem is onbruikbaar.
    \end{itemize}

    Daarom is validatie op één niveau nooit genoeg. Veel vormen van validatie zijn per definitie \emph{downstream}: je kunt een hoger niveau pas echt toetsen als de lagere niveaus al werken.

    \subsection{Belangrijkste aanbevelingen}

    Munzner doet drie expliciete aanbevelingen:
    \begin{enumerate}
        \item Maak duidelijk \emph{op welk niveau} je bijdrage zit, zeker als een paper meerdere niveaus raakt.
        \item Benoem expliciet welke aannames je maakt over hogere niveaus die je niet onderzoekt.
        \item Accepteer en waardeer meer papers die zich uitsluitend richten op domein- en probleemkarakterisatie.
    \end{enumerate}
    ...
    \section{misleiden met visualisaties}
    Het is heel makkelijk om mensen te misleiden met visualisaties.
    Bijvoorbeeld door de y-as niet bij 0 te laten beginnen, of door 3D-effecten te gebruiken die de verhoudingen vertekend weergeven.
    Ook kan je door kleuren en vormen te gebruiken die de aandacht afleiden van de data zelf.

    \subsection{Slecht ontwerp}
    Als een grafiek slecht ontworpen is, dan kan het (onbewust) misleidend zijn.
    Bijvoorbeeld door onduidelijke labels, onjuiste schalen of onlogische indelingen.
    3D-effecten kunnen ook de verhoudingen vertekend weergeven, waardoor de data anders lijkt dan het is.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{image/Slecht_ontwerp}
        \caption{}
        \label{fig:slecht_ontwerp}
    \end{figure}

    Links ziet er veel dramatischer uit dan rechts, terwijl de data hetzelfde is.
    Dit komt doordat de y-as bij links niet bij 0 begint, waardoor de verschillen groter lijken dan ze zijn.

    \subsection{Dubieuze data}
    Als de data zelf al dubieus is, dan kan de visualisatie ook misleidend zijn.
    Bijvoorbeeld door selectieve dataweergave, waarbij alleen de data wordt getoond die het gewenste verhaal ondersteunt.
    Of door het gebruik van onjuiste statistische methoden die de data verkeerd interpreteren.

    \subsection{Te weinig data}
    Als er te weinig data is, dan kan de visualisatie ook misleidend zijn.
    Bijvoorbeeld door het gebruik van kleine steekproeven die niet representatief zijn voor de populatie.
    Of door het gebruik van gemiddelden die de variabiliteit in de data verbergen.

    \subsection{verborgen of onduidelijke onzekerheid}
    Als de onzekerheid in de data niet duidelijk wordt weergegeven, dan kan de visualisatie ook misleidend zijn.
    Bijvoorbeeld door het weglaten van foutenmarges of betrouwbaarheidsintervallen.
    Of door het gebruik van grafieken die de onzekerheid verbergen, zoals lijngrafieken zonder foutbalken.

    \subsection{suggestieve patronen}
    Data kan soms patronen lijken te tonen die er in werkelijkheid niet zijn.
    Bijvoorbeeld door een grafiek te tonen die levensverwachting tegenover sigarettenconsumptie per land weergeeft.
    Dit kan de suggestie wekken dat er een verband is, terwijl er in werkelijkheid veel andere factoren meespelen.
    De visualisatie toont dan dat je langer leeft als je meer rookt.
    Dit komt, omdat rijke landen zowel een hogere levensverwachting als een hogere sigarettenconsumptie hebben.

    Ook kan je door toeval patronen zien die er niet zijn.
    Grappige voorbeelden hiervan kan je op de website \url{https://www.tylervigen.com/spurious-correlations} vinden.

    \section{interactieve visualisaties}
    Interactieve visualisaties zijn in sommige gevallen betern om:
    \begin{itemize}
        \item Grote datasets te verkennen
        \item Visueel verhaal te vertellen
        \item Data te verkennen zonder verhaal
        \item Mensen betrokkener maken bij de data
    \end{itemize}
    \\\\
    Er zijn verschillende soorten interacties die je kan gebruiken in interactieve visualisaties:
    \begin{itemize}
        \item \textbf{Exploreren} (zoomen, pannen, filteren)
        maakt het mogelijk om door de data te navigeren.
        Dit kan bijvoorbeeld door in- en uit te zoomen op een kaart, of door te pannen door een grote dataset.

        \item \textbf{Connecteren} (selecteren, highlighten)
        maakt het mogelijk om relaties tussen verschillende data-elementen te zien.
        Bijvoorbeeld door het highlighten van gerelateerde punten in een scatterplot als je een punt selecteert.

        \item \textbf{Filteren} (data subsetten)
        stelt gebruikers in staat om alleen de data te zien die relevant is voor hun analyse.
        Dit kan bijvoorbeeld door middel van dropdown-menu's of schuifregelaars.
        Je kan dan in een dataset of leeftijd van inwoners in Utrecht filteren op mannen of vrouwen,
        of mensen tussen de 20 en 30 jaar.

        \item \textbf{Configureren} (visualisatie aanpassen)
        laat gebruikers de visualisatie aanpassen aan hun voorkeuren.
        Dit kan bijvoorbeeld door het wijzigen van kleuren, vormen of groottes van de marks.
        kort: je kan de visualisatie personaliseren.

        \item \textbf{Encoderen} (data representatie aanpassen)
        maakt het mogelijk om de manier waarop data wordt weergegeven te veranderen.
        Bijvoorbeeld door het wisselen tussen een staafdiagram en een lijngrafiek.
        Of een scatterplot naar een dotplot.

        \item \textbf{Samenvatten/detailleren} (niveau van detail aanpassen)
        stelt gebruikers in staat om het niveau van detail in de visualisatie aan te passen.
        Dit kan bijvoorbeeld door het in- en uitzoomen op een kaart, of door het tonen van meer of minder gegevenspunten.

        \item \textbf{Selecteren} (items kiezen)
        maakt het mogelijk om specifieke data-elementen te kiezen voor verdere analyse.
        Dit kan bijvoorbeeld door het klikken op een punt in een scatterplot om meer informatie te zien over dat punt.
