...
% ====================================================
    ...
    * De ``?name'' betekendt hetzelde als ``<http://xmlns.com/foaf/0.1/name>'' maar dan korter geschreven.
    ...
    %TODO: 2 visualisaties, een met 15 random cirkels op een plot, en de ander met 3x5 cirkels in een grid.
    ...
    Een grafiek die visualiseert hoeveel mensen tijdens de oorlog van napoleon nog leefden na de veldslag:

    \begin{figure}[h]
        \centering
        \includegraphics[width=\linewidth]{image/Minard}
        \caption{grafiek oorlog napoleon}
        \label{fig:minard}
    \end{figure}

    \subsection{Algemene visualisatie principes}

    Sowieso: Vermijd onnodige 3D grafieken, omdat deze vaak verwarrend zijn en de interpretatie van de data bemoeilijken.

    \subsection{Principes van Edward Tufte}
    ...
    \begin{enumerate}
        ...
        \item Maximalisser redeliojkerwijs de \textbf{data-ink ratio} (de hoeveelheid inkt die daadwerkelijk data representeert versus de totale hoeveelheid inkt gebruikt in de visualisatie). $Data\text{-}ink\ ratio = \frac{Data\ ink}{Total\ ink}$
    ...
    \subsection{Wat is er te vinden in een visualisatie?}
    \begin{itemize}
        \item \textbf{Marks}: Dit is de data die je wilt visualiseren, zoals punten, lijnen of vlakken. In een bar chart zijn de balken de marks. In een scatter plot zijn de punten de marks.
        \item \textbf{Channels}: Dit zijn de visuele eigenschappen die je gebruikt om de marks te representeren, zoals kleur, grootte, vorm en positie. In een bar chart kan de hoogte van de balken de waarde van de data representeren, terwijl de kleur van de balken een categorie kan aangeven.
    \end{itemize}
