...
\begin{document}
    ...
    Het \textbf{cartesisch product} van twee sets A en B is de set van alle mogelijke paren (a, b) waarbij a een element is van A en b een element is van B.
    Dit schrijf je als $A \times B$, en stel A = {1, 2} en B = {x, y}, dan is $A \times B = \{(1,x), (1,y), (2,x), (2,y)\}$.
    ...
    \subsection{Bools algebra regels}
...
% --- diagram 3 (not a function) ---
    ...
    Een relatie is een manier om elementen van een set met elkaar te verbinden.
    Formeel is een relatie R op de sets A en B een subset van het Cartesisch product $A \times B$.
    Een relatie R tussen A en B schrijf je als $ARB$, zo ziet het er slecht te lezen uit, maar ``>'' is ook een relatie die je als $X>Y$ schrijft, dat is logischer te lezen.
    Je kan de relatie R ook schrijven als $R(A, B)$ of $(A,B) \in R$.

    Het verschil tussen functies en relaties, is dat een functie elke input koppelt aan precies één output, terwijl een relatie meerdere outputs kan hebben voor een enkele input, of zelfs geen output.
    Bijvoorbeeld, de relatie ``is ouder dan'' tussen mensen kan meerdere mensen omvatten die ouder zijn dan een bepaalde persoon, terwijl een functie zoals ``geef de leeftijd van'' elke persoon koppelt aan precies één leeftijd.

    De relatie R heeft ook een domein en een bereik.
    Het domein van R is de set van alle eerste elementen in de paren van R, en het bereik van R is de set van alle tweede elementen in de paren van R.
    Ofwel:\\
    $domain(R) = \{a \in A : \exists b \in B, (a,b) \in R\}$\\
    $range(R) = \{b \in B : \exists a \in A, (a,b) \in R\}$.

    \subsection{Eigenschappen van relaties}
    Relaties kunnen verschillende eigenschappen hebben, afhankelijk van hoe de elementen in de sets met elkaar verbonden zijn.
    Hier zijn enkele belangrijke eigenschappen van relaties:

    \begin{itemize}
        \item \textbf{Reflexief}: Een relatie R op een set A is reflexief als voor alle elementen a in A, het paar (a, a) in R zit.
        Bijvoorbeeld, de relatie ``is gelijk aan'' is reflexief omdat elke persoon gelijk is aan zichzelf.
        Als dit nooit het geval is, dan noem je het \textbf{irreflexief}.
        \item \textbf{Symmetrisch}: Een relatie R op een set A is symmetrisch als voor alle elementen a en b in A, als (a, b) in R zit, dan ook (b, a) in R zit.
        Bijvoorbeeld, de relatie ``is getrouwd met'' is symmetrisch omdat als persoon A getrouwd is met persoon B, dan is persoon B ook getrouwd met persoon A.
        \item \textbf{Antisymmetrisch}: Een relatie R op een set A is antisymmetrisch als voor alle elementen a en b in A, als (a, b) en (b, a) in R zitten, dan moet a gelijk zijn aan b.
        Bijvoorbeeld, de relatie ``is ouder dan'' is antisymmetrisch omdat als persoon A ouder is dan persoon B, en persoon B ouder is dan persoon A, dan moeten ze dezelfde leeftijd hebben (wat onmogelijk is).
        \item \textbf{Transitief}: Een relatie R op een set A is transitief als voor alle elementen a, b, en c in A, als (a, b) en (b, c) in R zitten, dan ook (a, c) in R zit.
        Bijvoorbeeld, de relatie ``is een voorouder van'' is transitief omdat als persoon A een voorouder is van persoon B, en persoon B een voorouder is van persoon C, dan is persoon A ook een voorouder van persoon C.
    \end{itemize}

    \subsection{equivalentie klassen}
    Je kan een equivalentie relatie gebruiken om een set op te delen in verschillende subsets, die we equivalentie klassen noemen.
    Elke equivalentie klasse bevat elementen die allemaal met elkaar gerelateerd zijn volgens de equivalentie relatie.
    Bijvoorbeeld, als we de set van auto's hebben en de relatie ``heeft dezelfde kleur als'', dan kunnen we de auto's opdelen in equivalentie klassen op basis van hun kleur.
    Dus alle rode auto's vormen één equivalentie klasse, alle blauwe auto's vormen een andere klasse, enzovoort.

    Deze kan je vervolgens opdelen in partities, waarbij elke element in de set precies in één subset zit.
    Dus in het bovenstaande voorbeeld, zou elke auto precies in één kleurklasse zitten, en deze zouden samen de hele set van auto's vormen zonder overlap.
    De definitie hiervan is $A_1 \cap A_2 = \emptyset$ voor alle $A_1, A_2$ in de partitie, waarbij $A_1 \neq A_2$.
    En $A_1 \cup A_2 \cup ... \cup A_n = A$.
    ...
    BNF is een manier om formeel de syntax van talen te definiëren.
    Het bestaat uit regels die aangeven hoe symbolen kunnen worden gecombineerd om geldige zinnen of expressies te vormen.
    Elke regel heeft een linkerkant (non-terminal symbool) en een rechterkant (combinatie van terminal en non-terminal symbolen).
    De notatie gebruikt meestal de volgende symbolen:
    \begin{itemize}
        \item \textbf{Non-terminal symbolen}: Dit zijn symbolen die verder kunnen worden uitgebreid of vervangen door andere symbolen. Ze worden meestal geschreven in hoekige haken, zoals <expression>, <term>, etc.
        \item \textbf{Terminal symbolen}: Dit zijn de basis symbolen die niet verder kunnen worden uitgebreid. Ze vertegenwoordigen de daadwerkelijke tekens of woorden in de taal, zoals cijfers, letters, of specifieke keywords.
        \item \textbf{Productie regels}: Dit zijn de regels die aangeven hoe non-terminal symbolen kunnen worden vervangen door combinaties van terminal en non-terminal symbolen. Ze worden meestal geschreven in de vorm: <non-terminal> ::= <combinatie van symbolen>.
    \end{itemize}
    \\\\
    ...
    \begin{multicols}{2}
        ...
        <Node> ::= b | N(<Node>, <Node>)
        \end{verbatim}
        Dit betekent dat een <Node> ofwel een blad (b) is, of een knoop (N) met twee sub-nodes.
        ...
        \begin{figure}[H]
            ...
            \resizebox{0.5\columnwidth}{!}{%
                \begin{tikzpicture}
                    \tikzstyle{every node}=[font=\LARGE]
                    \node [font=\LARGE] at (18.75,8.5) {N};
                    \node [font=\LARGE] at (21.25,8.5) {N};
                    \node [font=\LARGE] at (22.5,7.25) {N};
                    \node [font=\LARGE] at (20,9.75) {N};
                    \node [font=\LARGE] at (23.75,6) {N};
                    \draw (20.25,9.5) to[short] (20.75,9);
                    \draw (19.5,9.5) to[short] (19,9);
                    \draw (21,8.25) to[short] (20.5,7.75);
                    \draw (22.75,7) to[short] (23.25,6.5);
                    \draw (21.5,8.25) to[short] (22,7.75);
                    \draw (23.5,5.75) to[short] (23,5.25);
                    \draw (24,5.75) to[short] (24.5,5.25);
                    \draw (19,8.25) to[short] (19.5,7.75);
                    \draw (18.5,8.25) to[short] (18,7.75);
                    \draw (22.25,7) to[short] (21.75,6.5);
                    \node [font=\LARGE] at (21.25,6) {b};
                    \node [font=\LARGE] at (22.5,4.75) {b};
                    \node [font=\LARGE] at (20.75,7.25) {b};
                    \node [font=\LARGE] at (25,4.75) {b};
                    \node [font=\LARGE] at (19.25,7.25) {b};
                    \node [font=\LARGE] at (17.5,7.25) {b};
                \end{tikzpicture}
            }%
        \end{figure}
    \end{multicols}
    ...
    \end{verbatim}
    Deze regels geven aan dat een <expression> kan bestaan uit een <term>, of een <term> gevolgd door een '+' en nog een <expression>.
    Een <term> kan bestaan uit een <digit> of een <digit> gevolgd door nog een <term>.
    Een <digit> kan een van de cijfers 0 tot 9 zijn.
    \\\\
    ...
    \end{verbatim}
    Hierboven is een simpele programmeertaal gedefinieerd met toewijzingen, sequenties, conditionals en loops.
    Expressies kunnen getallen, variabelen en basis rekenkundige operaties bevatten.
    Booleaanse expressies kunnen waarheidswaarden, vergelijkingen en logische operaties bevatten

    \subsection{inductief gedefineerde functies}
    Net zoals je inductief een set kan definiëren, kan je ook inductief een functie definiëren.
    Dit doe je door een basis geval te definiëren en een inductie stap.
    Bijvoorbeeld, de factorial functie kan je zo definiëren:
    \begin{align*}
        0! &= 1; \quad \text{(basis geval)}\\
        n! &= n \times (n - 1)! \quad \text{voor } n > 0 \quad \text{(inductie stap)}
    \end{gather*}
    ...
    Recursieve functies zijn functies die zichzelf aanroepen om een probleem op te lossen.
    Je schrijft het als volgt:
    \[
        f(n) =
        \begin{cases}
            1, & \text{als } n \leq 0; \\
            f(n/2), & \text{als } n > 1 \text{ even}; \\
            f(3n + 1), & \text{als } n > 1 \text{ oneven}.
        \end{cases}
    \]

    In dit voorbeeld is de functie f gedefinieerd met drie gevallen:
    \begin{itemize}
        \item Als n kleiner dan of gelijk aan 0 is, retourneert f de waarde 1.
        \item Als n groter dan 1 is en even, roept f zichzelf aan met de waarde n gedeeld door 2.
        \item Als n groter dan 1 is en oneven, roept f zichzelf aan met de waarde 3n + 1.
    \end{itemize}
    Recursieve functies zijn krachtig omdat ze complexe problemen kunnen oplossen door ze op te splitsen in kleinere, beter beheersbare subproblemen.
    Het is echter belangrijk om ervoor te zorgen dat recursieve functies een basisgeval hebben om oneindige lussen te voorkomen.

    \section{Games}
    Een paar definitie's:
    \begin{itemize}
        \item \textbf{Game of chance}, Zoals roulette of dobbelstenen, waarbij de uitkomst volledig afhankelijk is van toeval.
        \item \textbf{Game of no chance}, Zoals boter-kaas-en-eieren of schaken, waarbij de uitkomst volledig afhankelijk is van de beslissingen van de spelers.
        \item \textbf{Perfect information}, Beide spelers weten alles wat er te weten valt over de staat van het spel op elk moment.
        \item \textbf{Winning strategy}, Een strategie die ervoor zorgt dat een speler altijd wint, ongeacht wat de tegenstander doet.
        \item \textbf{Drawing strategy}, Een strategie die ervoor zorgt dat een speler minstens gelijk speelt, ongeacht wat de tegenstander doet.
        \item \textbf{lossing strategy}, Een strategie die ervoor zorgt dat een speler altijd verliest, ongeacht wat de tegenstander doet.
    \end{itemize}
    ...
    Deze relaties kun je door middel van een formule weergeven.
    Bijvoorbeeld:
    $\frac{}{0\le n}\le-Base$ en $\frac{n \le m}{s(n) \le s(m)}\le-Step$
    Als je hier wilt bewijzen dat $2 \le 5$, dan kan je dat zo doen:
    \[
        \frac{
            \frac{
                \frac{}{
            ...
            s(s(0)) \le s(s(s(s(s(0)))))
        }
        \le-Step
    \]

    Je zet als het ware elke stap in een breuk, waarbij de bovenkant de premissen zijn en de onderkant de conclusie.
    En dit Kan je ``nesten'' totdat je bij de stelling komt die je wilt bewijzen.
    \\\\

    Je kan dit ook in een tabel zetten:\\
    \begin{SimpleTable}[s{.5}s{.5}]{}{}
        \TableHeader{Stelling & Reden}
        \TableRow{$0 \le s(s(s(0)))$ & $\le-Base$}
        \TableRow{$s(0) \le s(s(s(s(0))))$ & $\le-Step$ op 1}
        \TableRow{$s(s(0)) \le s(s(s(s(s(0)))))$ & $\le-Step$ op 2}
    \end{SimpleTable}

    Hoe werkt dit nou?
    Het is de bedoeling dat je met de regels die je hebt, een stelling kunt bewijzen.
    Elke regel heeft een aantal premissen (boven de streep) en een conclusie (onder de streep).
    Als je alle premissen kunt bewijzen, dan mag je de conclusie aannemen.
    Dit doe je net zolang totdat je de stelling die je wilt bewijzen hebt bereikt.
    De rede dat ik gelijk $0 \le s(s(s(0)))$ mag aannemen is omdat dit de basis is van de inductie.
    $s(0)$ is (per definitie) wat komt ná $0$, dus $s(0) = 1$, daarom is $0 \le s(0)$ altijd waar en mag je dat aannemen.
    \\\\
    \begin{voorbeeld}{Even en Oneven getallen}
        Stel we defineren de Base en Step regels voor Even en Oneven getallen als volgt:
        \begin{gather*}
            \frac{}{\text{isEven}(0)}\text{isEven-Base}\\
            \frac{isEven(n)}{isEven(s(s(n)))}\text{isEven-Step}\\
        \end{gather*}

        Hieruit kan je bijvoorbeeld opmaken dat $s(s(s(s(0))))$ even is (makkelijker in tabelvorm):
        \begin{SimpleTable}[s{.5}s{.5}]{}{}
            \TableHeader{Stelling & Reden}
            \TableRow{$isEven(0)$ & $isEven-Base$}
            \TableRow{$isEven(s(s(0)))$ & $isEven-Step$ op 1}
            \TableRow{$isEven(s(s(s(s(0)))))$ & $isEven-Step$ op 2}
        \end{SimpleTable}
    \end{voorbeeld}
    ...
    \begin{voorbeeld}{isSorted}
        Stel we defineren 3 regels voor isSorted (isSorted-empty, isSorted-singleton, isSorted-step) als volgt:
        \begin{gather*}
            \frac{}{\text{isSorted}([])}\text{isSorted-empty}\\
            \frac{}{\text{isSorted}(n : [])}\text{isSorted-singleton}\\
            \frac{n \le m \quad \text{isSorted}(m : w)}{\text{isSorted}(n : m : w)}\text{isSorted-step}\\
        \end{gather*}
        ...
        \begin{SimpleTable}[s{.5}s{.5}]{}{}
            \TableHeader{Stelling & Reden}
            \TableRow{$isSorted([])$ & $isSorted-empty$}
            \TableRow{$isSorted(7 : [])$ & $isSorted-singleton$}
            \TableRow{$isSorted(5 : 7 : [])$ & $isSorted-step$ op 2, omdat $5 \le 7$}
            \TableRow{$isSorted(2 : 5 : 7 : [])$ & $isSorted-step$ op 3, omdat $2 \le 5$}
            \TableRow{$isSorted(0 : 2 : 5 : 7 : [])$ & $isSorted-step$ op 4, omdat $0 \le 2$}
        \end{SimpleTable}
        \\\\
        Maar wat nou als n \textbf{niet} kleiner is dan m (ofwel de lijst is niet gesorteerd)?
        Dan kan je dat ook bewijzen (op de lijst $[0, 2, 7, 5]$):
        \\
        \begin{SimpleTable}[s{.5}s{.5}]{}{}
            \TableHeader{Stelling & Reden}
            \TableRow{$isSorted([])$ & $isSorted-empty$}
            \TableRow{$isSorted(5 : [])$ & $isSorted-singleton$}
            \TableRow{(geen regel toepasbaar op $isSorted(7 : 5 : [])$) & $7 \nleq 5$}
        \end{SimpleTable}
    \end{voorbeeld}

    \subsection{natuurlijke deductie in propositielogica (Derivation)}
    Als we $P \implies Q$ wilt bewijzen, dan neem je aan dat P waar is, en dan bewijs je Q.
    Maar P is alleen een geldige aannname binnen de context van dit specifieke bewijs en niet daarbuiten.

    Om dit te bewijzen moeten we dus ook buiten de context van P kunnen bewijzen dat P waar is.
    Dit wordt vervolgens bijgehouden in een Set, een set met alle aannames die we hebben gemaakt.
    Zo kan je dus in de set zetten: $\Gamma\vdash P$ wat in woorden ``Uit de aannames in de set $\Gamma$ volgt dat P waar is'' betekend.
    Je kan tijdens het bewijzen ook nieuwe aannames maken, deze worden dan toegevoegd aan de set.


    Als je die breuk-achtige dingen maakt, dan zijn het zegmaar herschrijf regels.
    Je hebt dan allemaal regels die al kant en klaar zijn en die moet je dan invullen met de data die je hebt
    dus bijvoorbeeld:

    $\frac{P Q}{P \land Q}$, P moet waar zijn en Q moet waar zijn en dan is P en Q waar.
    Maar je kan voor P bijvoorbeeld ook $P = A \lor B$ invullen, en dan moet je dus bewijzen dat $A \lor B$ waar is.
    dan krijg je een nieuwe regel:
    $\frac{A \lor B Q}{(A \lor B) \land Q}$
    Die A of B is dan op zichzelf ook weer een breuk en zo gaat dat door.

    Een lijst van alle regels van natuurlijke deductie in propositielogica:
    \begin{multicols}{3}
        \[\frac{P \in \Gamma}{\Gamma \vdash P}\text{-Assumption}\] ``Als P in de aannameset zit, dan volgt P''\\
        \[\frac{}{\Gamma \vdash \top}\text{T-Introduction}\] ``Waarheid is altijd waar''\\
        \[\frac{\Gamma \vdash \bot}{\Gamma \vdash P}\text{F-Elimination}\] ``Als je een onwaarheid hebt, kan je alles bewijzen''\\
        \[\frac{\Gamma, P \vdash Q}{\Gamma \vdash P \implies Q}\text{Implication-Introduction}\] ``Als je vanuit P kan bewijzen dat Q waar is, dan volgt dat P impliceert Q''\newcolumn
        \[\frac{\Gamma \vdash P \quad \Gamma \vdash Q}{\Gamma \vdash P \land Q}\text{And-Introduction}\] ``Als P waar is en Q waar is, dan is P en Q waar''\\
        \[\frac{\Gamma \vdash P \implies Q \quad \Gamma \vdash P}{\Gamma \vdash Q}\text{Implication-Elimination}\] ``Als P Q impliceert, en P waar is, dan volgt Q''\\
        \[\frac{\Gamma \vdash P \land Q}{\Gamma \vdash P}\text{And-Elimination-1}\] ``Als P en Q waar is, dan is P waar''\\
        \[\frac{\Gamma \vdash P \land Q}{\Gamma \vdash Q}\text{And-Elimination-2}\] ``Als P en Q waar is, dan is Q waar''\newcolumn
        \[\frac{\Gamma \vdash P}{\Gamma \vdash P \lor Q}\text{Or-Introduction-1}\] ``Als P waar is, dan is P of Q waar''\\
        \[\frac{\Gamma \vdash Q}{\Gamma \vdash P \lor Q}\text{Or-Introduction-2}\] ``Als Q waar is, dan is P of Q waar''\\
        \[\frac{\Gamma, P \vdash R \quad \Gamma, Q \vdash R}{\Gamma \vdash P \lor Q \implies R}\text{Or-Elimination}\] ``Als vanuit P kan bewijzen dat R waar is, en vanuit Q kan bewijzen dat R waar is, dan volgt dat P of Q R impliceert''\newcolumn
        \[\frac{\Gamma \vdash P}{\Gamma \vdash \neg \neg P}\text{Negation-Introduction}\] ``Als P waar is, dan is niet niet P waar''\\
        \[\frac{\Gamma \vdash \neg \neg P}{\Gamma \vdash P}\text{Negation-Elimination}\] ``Als niet niet P waar is, dan is P waar''\\
        \[\frac{\Gamma, P \vdash Q \quad \Gamma, Q \vdash \bot}{\Gamma \vdash P \Leftrightarrow Q}\text{Biconditional-Introduction}\] ``Als vanuit P kan bewijzen dat Q waar is, en vanuit Q kan bewijzen dat onwaarheid waar is, dan volgt dat P als en slechts als Q''\newcolumn
        \[\frac{\Gamma \vdash P \Leftrightarrow Q}{\Gamma \vdash P \implies Q}\text{Biconditional-Elimination-1}\] ``Als P als en slechts als Q waar is, dan volgt dat P Q impliceert''\\
        \[\frac{\Gamma \vdash P \Leftrightarrow Q}{\Gamma \vdash Q \implies P}\text{Biconditional-Elimination-2}\] ``Als P als en slechts als Q waar is, dan volgt dat Q P impliceert''\newcolumn
        \[\frac{\Gamma, \neg P \vdash \bot}{\Gamma \vdash P}\text{Proof by Contradiction}\] ``Als vanuit niet P kan bewijzen dat onwaarheid waar is, dan volgt dat P waar is''
    \end{multicols}

    Dit zijn alle natuurlijke deductie regels die je nodig hebt om propositielogica te bewijzen.
    Deze kan je vervolgens samenvoegen om complexere stellingen te bewijzen.

    \begin{voorbeeld}{$P \land Q \imp$}
    \end{voorbeeld}


    \section{Redenering over progamma's}
    ...
    \begin{itemize}
        \item $\sigma$: een staat, die de waarden van alle variabelen op een bepaald moment in het programma vertegenwoordigt.
        Je schrijft dit als een functie die variabelen naar hun waarden mappt.
        \item $\langle p, \sigma \rangle \rightarrow \sigma'$: Dit is de operationele semantiek, die beschrijft hoe een programma $p$ wordt uitgevoerd in een staat $\sigma$ en resulteert in een nieuwe staat $\sigma'$.
        \item $[[p]](\sigma) = \sigma'$: Dit is de denotationale semantiek, die een programma $p$ beschrijft als een functie die een staat $\sigma$ naar een nieuwe staat $\sigma'$ mapt.
    \end{itemize}
    ...
    \]
    Deze regel zegt dat als de expressie $e$ in de staat $\sigma$ evalueert naar de waarde $n$, dan zal het toewijzingscommando $x := e$ in de staat $\sigma$ resulteren in een nieuwe staat $\sigma'$ waarin de variabele $x$ de waarde $n$ heeft.
    ...
    \]
    Deze regel zegt dat als de booleaanse expressie $b$ in de staat $\sigma$ evalueert naar waar, en het commando $p$ in de staat $\sigma$ resulteert in een nieuwe staat $\sigma'$, dan zal het if-commando in de staat $\sigma$ ook resulteren in de staat $\sigma'$.
    ...
    \]
    Deze regel zegt dat als de booleaanse expressie $b$ in de staat $\sigma$ evalueert naar onwaar, en het commando $q$ in de staat $\sigma$ resulteert in een nieuwe staat $\sigma'$, dan zal het if-commando in de staat $\sigma$ ook resulteren in de staat $\sigma'$.

    \begin{voorbeeld}{een conditional uitvoeren}
        Stel dat we de volgende staat hebben: $\sigma(x) = 3, \quad \sigma(y) = 10$
        En het volgende programma: \verb|if x < y then z := x + 1 else z := y + 1|
        Als we hier de derivatie regels op toepassen, krijgen we het volgende:
        \\\\
        \[
            \frac{
                \llbracket x < y \rrbracket(\sigma) = true \quad
                \frac{
                    \llbracket x \rrbracket(\sigma) = 3
                }
                {
                    \langle r := x, \sigma \rangle \rightarrow \sigma[r \mapsto 3]
                }
            }
            {
                \langle \text{if } x < y \text{ then } r := x \text{ else } r := y, \sigma \rangle \rightarrow \sigma[r \mapsto 3]
            }
        \]
    \end{voorbeeld}

    \section{Hoare logica}
    Hoare logica is bedoeld om formeel te redeneren over de correctheid van computerprogramma's.
    Het maakt gebruik van zogenaamde Hoare triples, die de vorm hebben: $\{P\} C \{Q\}$.
    Hierin is P de preconditie, C het commando (of programma), en Q de postconditie.
    De betekenis van een Hoare triple is dat als de preconditie P waar is voordat het commando C wordt uitgevoerd, dan zal de postconditie Q waar zijn nadat C is uitgevoerd, op voorwaarde dat C correct wordt uitgevoerd (d.w.z. zonder fouten of uitzonderingen).
    \\$\{P\} C \{Q\} \leftarrow$ ``Voor elke staat $\sigma$ die voldoet aan P, als we $\langle p, \sigma \rangle$ uitvoeren (het commando p in de staat $\sigma$) en dit leidt tot een nieuwe staat $\tau$, dan voldoet $\tau'$ aan Q.''

    \begin{voorbeeld}{Hoare triple}
        Stel we hebben de volgende Hoare triple:
        \[
            \{x = 3\} \quad y := x + 1 \quad \{y = 4\}
        \]
        Deze triple zegt dat als de preconditie $x = 3$ waar is voordat we het commando $y := x + 1$ uitvoeren, dan zal de postconditie $y = 4$ waar zijn nadat het commando is uitgevoerd.
        Laten we dit stap voor stap bekijken:
        \begin{itemize}
            \item Preconditie: We beginnen met de aanname dat $x = 3$.
            \item Commando: We voeren het commando $y := x + 1$ uit. Dit betekent dat we de waarde van $x$ (die 3 is) nemen en er 1 bij optellen, waardoor $y$ de waarde 4 krijgt.
            \item Postconditie: Na het uitvoeren van het commando, hebben we $y = 4$. Dit voldoet aan de postconditie die we wilden bewijzen.
        \end{itemize}
        Dus de Hoare triple is correct, omdat de preconditie leidt tot de postconditie na het uitvoeren van het commando.

        \\\\\\

        Een ander voorbeeld is de volgende Hoare triple:
        \[
            \{x = A \land y = B\} \quad z := x; x := y; y := z \quad \{x = B \land y = A\}
        \]
        Deze triple beschrijft een commando dat de waarden van $x$ en $y$ verwisselt.
        Laten we dit stap voor stap bekijken:
        \begin{itemize}
            \item Preconditie: We beginnen met de aanname dat $x = A$ en $y = B$.
            \item Commando: We voeren het commando $z := x; x := y; y := z$ uit.
            Dit betekent dat we eerst de waarde van $x$ opslaan in een tijdelijke variabele $z$, vervolgens de waarde van $y$ toewijzen aan $x$, en ten slotte de waarde van $z$ (die oorspronkelijk $x$ was) toewijzen aan $y$.
            \item Postconditie: Na het uitvoeren van het commando, hebben we $x = B$ en $y = A$.
            Dit voldoet aan de postconditie die we wilden bewijzen.
        \end{itemize}
        Dus de Hoare triple is correct, omdat de preconditie leidt tot de postconditie na het uitvoeren van het commando.
        ...
        \[
            \{true\} \text{while true do p := 0 od} \{p = 500\}
        \]
        Deze triple beschrijft een oneindige lus die altijd het commando $p := 0$ uitvoert.
        Laten we dit stap voor stap bekijken:
        \begin{itemize}
            \item Preconditie: We beginnen met de aanname dat de preconditie altijd waar is (aangeduid door $true$).
            \item Commando: We voeren het commando $while true do p := 0$ od uit.
            Dit betekent dat we een oneindige lus hebben die altijd het commando $p := 0$ uitvoert.
            \item Postconditie: Na het uitvoeren van het commando, hebben we $p = 500$.
            Dit voldoet niet aan de postconditie die we wilden bewijzen, omdat $p$ altijd 0 zal zijn na het uitvoeren van het commando.
        \end{itemize}
        Dus de Hoare triple is niet correct, omdat de preconditie niet leidt tot de postconditie na het uitvoeren van het commando.
    \end{voorbeeld}
