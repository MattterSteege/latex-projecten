...
% --- diagram 3 (not a function) ---
    ...
    \subsection{aannames incompleet}
    ...
    \subsection{natuurlijke deductie in propositielogica incompleet}
    Je kan dit doen aan de hand van een waarheidstabel zoals helemaal in het begin besproken is.
    Je kent nog wel $p$ en $q$, deze kan je in BNF (Backus-Naur Form) schrijven als:
    $p, q ::= true | false | p \land q | p \lor q | \neg p | p \implies q | p \Leftrightarrow q$
    \begin{itemize}
        \item Als p waar is: \textbf{T}
        \item Als p onwaar is: \textbf{F}
        \item Wat nou als p een variabele is?
        Dan kan die zowel T als F zijn.
        Dan gebruiken we een functie die alle mogelijke waardes van p en q neemt en daar de uitkomst van de formule op toepast.
    \end{itemize}

    Deze functie: $v : P \rightarrow Bool$, waarbij P een propositonele formule is, en Bool uitkomst.
